...
export default function Card({ sm, title, desc, url }) {
  ...
  return (
    <div className={`overflow-hidden flex lg:flex-col justify-between gap-y-8 gap-x-12 motion-safe:animate-fadeIn px-10 py-6 rounded-xl shadow-xl text-gray-800 bg-mistyRose-500 
    ${sm ? "lg:h-3/5 lg:w-2/4" : "lg:h-5/6 lg:w-2/3"}`}>
      ...
      <section className='text-bluePigment-500 flex flex-col gap-y-4 justify-between'>
        ...
        <a target="_blank" rel='noreferrer' className='w-full px-2 py-2 lg:px-6 lg:py-3 text-sm lg:text-xl font-semibold rounded-2xl shadow-2xl bg-bluePigment-500 text-gray-100' href={url}>See more</a>
      </section>
    </div>
  )
}
